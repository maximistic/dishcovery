...
export const fetchRecipesByMealType = async (mealType: string) => {
  const API_KEY = process.env.NEXT_PUBLIC_SPOONACULAR_API_KEY;
  const API_URL = `https://api.spoonacular.com/recipes/complexSearch?mealType=${mealType}&number=4&fillIngredients=true&apiKey=${API_KEY}`;
